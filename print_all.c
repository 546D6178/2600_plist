...
void print_all()
{
    ...
    _tprintf(_T("Process ID\tElapsed Time\n"));
    ...
    { 
        ...
        //_tprintf( TEXT("%zu\t%02d:%02d:%02d:%03d"), pmc.WorkingSetSize / 1024, creationSystemTime.wHour, creationSystemTime.wMinute, creationSystemTime.wSecond, creationSystemTime.wMilliseconds ); //Priv : Taille actuelle du jeu de travail, en Kilo-octets ou plutôt Kibi-octets (Kio) . 1024 = 2 puissance 10
    ...
    CloseHandle(hSnapshot); //ferme le handle pour finir le script
}